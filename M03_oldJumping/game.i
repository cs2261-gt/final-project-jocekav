...
void updatePig() {
 ...
 if ((~((*(volatile unsigned short *)0x04000130)) & ((1<<4)))) {
  pig.velx = 2;
  if (pig.screenCol + pig.width + 2 > 240) {
   pig.velx = 0;
  }
 ...
 if ((!(~(oldButtons)&((1<<6))) && (~buttons & ((1<<6)))) && !pig.midAir) {
  pig.vely = -7;
  pig.midAir = 1;
  pig.timeInAir = 1;
 }
 ...
 if (pig.screenRow >= (160 - pig.height) && flag) {
  loser = 1;
 }
}
...
void updateClouds() {
 for (int i = 0; i < 3; i++) {
  ...
  if (clouds[i].active && ((clouds[i].screenRow + clouds[i].height/2) > 160)){
   clouds[i].active = 0;
  }
  ...
  if (!clouds[i].active) {
   ...
   if ((clouds[i].screenCol + clouds[i].width) >= 240) {
    clouds[i].screenCol = 240 - clouds[i].width ;
   }
   clouds[i].counter = 0;
   if (rand() % 3 == 0) {
    clouds[i].hasCoin = 1;
...
void initClouds() {
 ...
 for (int i = 0; i < 3; i++) {
  ...
  clouds[i].hide = 0;
  clouds[i].aniState = 0;
  cloud[i].aniCounter = 0;
  clouds[i].hasWings = 0;
  getCloudDim(&clouds[i]);
  row -= 50;
 }
 clouds[2].disappear = 1;
 topRow = clouds[3].screenRow;
 topCol = 40;
}
...
void drawPig() {
 shadowOAM[0].attr0 = pig.screenRow | (0<<14);
 shadowOAM[0].attr1 = pig.screenCol | (2<<14);
   ...
   switch (clouds[i].type) {
    case XL:
     ...
     if (clouds[i].hide) {
      ...
      shadowOAM[i + 1].attr0 = clouds[i].screenRow | (1<<14);
     }
     shadowOAM[i + 1].attr1 = clouds[i].screenCol | (3<<14);

     if (clouds[i].aniState == 0) {
      shadowOAM[i + 1].attr2 = ((0)*32+(4));
     } else if (clouds[i].aniState == 1) {
      shadowOAM[i + 1].attr2 = ((4)*32+(4));
    ...
    case LG:
              shadowOAM[i + 1].attr0 = clouds[i].screenRow | (1<<14);
     shadowOAM[i + 1].attr1 = clouds[i].screenCol | (2<<14);
     shadowOAM[i + 1].attr2 = ((0)*32+(12));
              break;
    case MED:
     shadowOAM[i + 1].attr0 = clouds[i].screenRow | (1<<14);
     shadowOAM[i + 1].attr1 = clouds[i].screenCol | (1<<14);
     shadowOAM[i + 1].attr2 = ((0)*32+(16));
              break;
    case SM:
     shadowOAM[i + 1].attr0 = clouds[i].screenRow | (1<<14);
     shadowOAM[i + 1].attr1 = clouds[i].screenCol | (0<<14);
     shadowOAM[i + 1].attr2 = ((0)*32+(20));
              break;
   }
  }
 }
...
void drawCoins() {
 ...
 if (coin.active) {
   shadowOAM[2 + 3].attr0 = coin.screenRow | (0<<14);
   shadowOAM[2 + 3].attr1 = coin.screenCol | (1<<14);

   switch (coin.type) {
    case GOLD:
     shadowOAM[2 + 3].attr2 = ((0)*32+(22));
     break;
    case SILVER:
     shadowOAM[2 + 3].attr2 = ((0)*32+(24));
     break;
    case BRONZE:
     shadowOAM[2 + 3].attr2 = ((0)*32+(26));
     break;
 }
 } else {
  shadowOAM[2 + 3].attr0 = coin.screenRow | (2<<8);
 }
}
...
void coinCollisions() {
 ...
 if (coin.active && coin.screenRow + coin.height >= 160) {
  coin.active = 0;
 }
}
...
void drawScore() {
 shadowOAM[30].attr0 = 0 | (0<<14);
 shadowOAM[30].attr1 = 0 | (0<<14);
 shadowOAM[30].attr2 = (((thousands + 4))*32+(0));

 shadowOAM[31].attr0 = 0 | (0<<14);
 shadowOAM[31].attr1 = 8 | (0<<14);
 shadowOAM[31].attr2 = (((hundreds + 4))*32+(0));

 shadowOAM[32].attr0 = 0 | (0<<14);
 shadowOAM[32].attr1 = 16 | (0<<14);
 shadowOAM[32].attr2 = (((tens + 4))*32+(0));

 shadowOAM[33].attr0 = 0 | (0<<14);
 shadowOAM[33].attr1 = 24 | (0<<14);
 shadowOAM[33].attr2 = (((ones + 4))*32+(0));
}
...
void drawWings() {
 ...
 if (wings.active) {
   shadowOAM[3 + 3].attr0 = wings.screenRow | (0<<14);
   shadowOAM[3 + 3].attr1 = wings.screenCol | (1<<14);
   shadowOAM[3 + 3].attr2 = ((0)*32+(28));
 } else {
  shadowOAM[3 + 3].attr0 = wings.screenRow | (2<<8);
 }
}
...
void wingsCollisions() {
 ...
 if (wings.active && wings.screenRow + wings.height >= 160) {
  wings.active = 0;
 }
}
